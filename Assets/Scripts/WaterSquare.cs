...
public class WaterSquare : MonoBehaviour {
	...
	public void Init(GameObject waterSquareObj, float size, float spacing)
	{
		...
		// Calculate the data we need to gen mesh
		width = (int)(size / spacing);
		// because each square is 2 vertices, we need 1 more
		width += 1;
		...
		// Save the center of the square
		this.centerPos = waterSquareObj.transform.localPosition;

		// Generate Sea

		// Calculate the time it took to generate the terrain
	...
	// If we are updateding the square from outside of a thread
	public void MoveSea(Vector3 oceanPos, float timeSinceStart)
	{
		...
		for (int i = 0; i < vertices.Length; i++)
		{
			...
			// Get the water height at this coord
			...
			// Don't need to go from global to local because y always = 0
			vertices [i] = vertex;
		}
	...
	public void GenerateMesh()
	{
		...
		for (int z = 0; z<width;z++)
		{
			...
			for(int x = 0; x < width; x++)
			{
				...
				// Don't generate a Triangle for the first coord as it is only 1 point
				if (x <= 0 || z <= 0)
					continue;

				// Each square is 2 triangles

				// SW triangle vertices
				tris.Add (x + z * width);
		...
		// Unfold the 2d array into 1d array
		...
		// Ensure bounding volume is correct
		newMesh.RecalculateBounds ();
		// update normals
		newMesh.RecalculateNormals ();

		// Add the gen mesh to the gameobject
